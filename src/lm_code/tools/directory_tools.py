    ...
    def execute(self, dir_path: str) -> str:
        ...
        try:
            ...
            if ".." in dir_path.split(os.path.sep):
                log.warning(
                    f"Attempted to access parent directory in create_directory path: {dir_path}"
                )
    ...
    def execute(self, path: str | None = None) -> str:
        ...
        if path:
            ...
            if target_path.startswith(".."):
                ...
                return (
                    f"Error: Invalid path '{path}'. Cannot access parent directories."
                )
        ...
        try:
            ...
            if process.returncode == 0:
                log.info(
                    f"Directory listing command successful for path '{target_path}'."
                )
                output = process.stdout.strip()
                if len(output.splitlines()) > 100:
                    log.warning(
                        f"Directory listing output for '{target_path}' exceeded 100 lines. Truncating."
                    )
                    output = (
                ...
                if (
                    ...
                    log.error(
                        f"Directory listing command failed: Directory not found '{target_path}'. Stderr: {process.stderr.strip()}"
                    )
                    return f"Error: Directory not found: '{target_path}'"
                else:
                    log.error(
                        f"Directory listing command failed with return code {process.returncode}. Path: '{target_path}'. Stderr: {process.stderr.strip()}"
                    )
                    error_detail = (
                        process.stderr.strip() if process.stderr else "(No stderr)"
                    )
                    return f"Error executing directory listing command (Code: {process.returncode}): {error_detail}"

        except FileNotFoundError:
            log.error(
                "Directory listing command not found. Ensure it is installed and in the system's PATH."
            )
            return "Error: Directory listing command not found. Please ensure it is installed and in the system's PATH."
        except subprocess.TimeoutExpired:
            log.error(
                f"Directory listing command timed out for path '{target_path}' after 15 seconds."
            )
            return (
                f"Error: Directory listing command timed out for path '{target_path}'."
            )
        except Exception as e:
            log.exception(
                f"An unexpected error occurred while executing directory listing command for path '{target_path}': {e}"
            )
