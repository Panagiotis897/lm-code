    ...
    def execute(self, path: str | None = None, depth: int | None = None) -> str:
        ...
        try:
            ...
            if process.returncode == 0:
                log.info(
                    f"Tree command successful for path '{target_path}' with depth {depth_limit}."
                )
                # Limit output size? Tree can be huge.
                output = process.stdout.strip()
                if len(output.splitlines()) > 200:  # Limit lines as a proxy for size
                    log.warning(
                        f"Tree output for '{target_path}' exceeded 200 lines. Truncating."
                    )
                    output = (
                ...
                or "command not found" in process.stderr.lower()
            ):
            ...
            log.error(
                f"'tree' command not found (FileNotFoundError). It might not be installed."
            )
            return "Error: 'tree' command not found. Please ensure it is installed and in the system's PATH."
        except subprocess.TimeoutExpired:
            log.error(
                f"Tree command timed out for path '{target_path}' after 15 seconds."
            )
            return f"Error: Tree command timed out for path '{target_path}'. The directory might be too large or complex."
        except Exception as e:
            log.exception(
                f"An unexpected error occurred while executing tree command for path '{target_path}': {e}"
            )
