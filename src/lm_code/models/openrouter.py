...
class OpenRouterModel:
    ...
    def __init__(
        ...
        api_key: str,
        console: Console,
        model_name: str = "qwen/qwen-2.5-coder-32b-instruct:free",
    ):
        """Initialize the OpenRouter model interface."""
        if not api_key or not api_key.strip():
            raise ValueError(
                "API key is required. Run 'lmcode setup YOUR_OPENROUTER_API_KEY' first."
            )
        ...
        self.openrouter_tools = (
            self._convert_to_openrouter_tools() if self.function_declarations else None
        )
        # ---
        ...
        try:
            ...
            log.info(
                "OpenRouterModel initialized successfully (Native Function Calling Agent Loop)."
            )
        except Exception as e:
            log.error(
                f"Fatal error initializing OpenRouter model '{self.current_model_name}': {str(e)}",
                exc_info=True,
            )
    ...
    def _test_model_connection(self):
        ...
        try:
            ...
            if "text/html" in content_type:
                raise Exception(
                    f"API returned HTML instead of JSON. Check your API key and URL. Response: {response.text[:200]}"
                )
            ...
            try:
                ...
                if "choices" not in data:
                    log.warning(
                        f"API response missing 'choices' field: {list(data.keys())}"
                    )
    ...
    def generate(self, prompt: str) -> str | None:
        ...
        try:
            while iteration_count < MAX_AGENT_ITERATIONS:
                ...
                try:
                    logging.info(
                        f"Sending request to LLM ({self.current_model_name}). History length: {len(self.chat_history)} turns."
                    )
                    ...
                    with self.console.status(
                        ...
                        if not response.text or not response.text.strip():
                            log.error("API returned empty response body")
                            raise Exception(
                                "API returned an empty response. The model may be unavailable or rate-limited."
                            )
                        ...
                        try:
                            ...
                            log.error(
                                f"Failed to parse API response as JSON: {response.text[:500]}"
                            )
                            raise Exception(
                                f"Invalid JSON response from API: {response.text[:200]}"
                            )
                    # === END STATUS ===

                    # === START DEBUG LOGGING ===
                    log.debug(
                        f"RAW OpenRouter Response Object (Iter {iteration_count}): {llm_response}"
                    )
                    # === END DEBUG LOGGING ===
                    ...
                    if not llm_response.get("choices"):
                        log.error(
                            f"LLM response had no choices. Response: {llm_response}"
                        )
                    ...
                    if (
                        "tool_calls" in response_message
                    ...
                    # Handle text responses
                        ...
                        log.info(
                            f"LLM returned text (Iter {iteration_count}): {llm_text[:100]}..."
                        )
                        ...
                        last_text_response = (
                            "(Agent received response with no actionable content)"
                        )
                        task_completed = (
    ...
    def _manage_context_window(self):
        ...
        if len(self.chat_history) > (
            ...
            log.warning(
                f"Chat history length ({len(self.chat_history)}) exceeded threshold. Truncating."
            )
            # Keep system message (idx 0)
            keep_count = (
                MAX_HISTORY_TURNS * 2
            )  # Keep N rounds (user+assistant or user+tool)
            keep_from_index = len(self.chat_history) - keep_count
            self.chat_history = [self.chat_history[0]] + self.chat_history[
                keep_from_index:
            ]
            ...
            if hasattr(tool_instance, "get_function_declaration"):
                ...
                if isinstance(declaration, dict):  # Handle unexpected dictionary type
                    if "name" in declaration and "description" in declaration:
                        schema = {
                            "type": "object",
                            "properties": declaration.get("parameters", {}).get(
                                "properties", {}
                            ),
                            "required": declaration.get("parameters", {}).get(
                                "required", []
                            ),
                        }
                        declarations.append(
                        ...
                        log.debug(
                            f"Generated FunctionDeclaration (dict) for tool: {tool_name}"
                        )
                    else:
                        log.warning(
                            f"Unexpected dictionary format for tool declaration: {declaration}"
                        )
                    ...
                    if hasattr(declaration, "parameters") and declaration.parameters:
                        if hasattr(declaration.parameters, "properties"):
                            for (
                                prop_name,
                                prop_details,
                            ) in declaration.parameters.properties.items():
                        ...
                        if (
                            hasattr(declaration.parameters, "required")
                            and declaration.parameters.required
                        ):
                    ...
                    log.debug(
                        f"Generated FunctionDeclaration (object) for tool: {tool_name}"
                    )
                else:
                    log.warning(
        ...
        if self.function_declarations:
            for func_decl in self.function_declarations:
                ...
                if (
                    ...
                    and "properties" in func_decl.parameters
                ):
                    params_list = []
    ...
    def _find_last_model_text(self, history: list) -> str:
        ...
        for i in range(len(history) - 1, -1, -1):
            if (
                ...
                and "content" in history[i]
                and history[i]["content"]
            ):
